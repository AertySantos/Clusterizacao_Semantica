...
"""
# Clustering
_ALGORITHMS = [ "agglomerative", "kmeans","spectral", "dbscan","spectral"] # "spectral", "dbscan", "kmeans"
...
def load_matrix(filename, fformat, pathto="./data/IN/", fsep=" ", header=0):
    ...
    if fformat == "dense":
        ...
        try:
            data = np.array(
            [[float(value) for value in line.split(fsep)] for line in docs if line.strip()]
        )
            print(data)
    ...
    try:
        ...
        if os.stat(args.dbinput+args.dataset):
            ...
            if data is None or data.size == 0:
                raise ValueError("[Error] Matriz de dados carregada está vazia ou mal formatada.")
            if len(data.shape) != 2:
                raise ValueError(f"[Error] Formato inesperado: {data.shape}. Esperado: matriz 2D.")
